...
class MedicationModelTests(TestCase):
    ...
    def test_create_medication_zero_dosage(self):
        ...
        try:
            med = Medication.objects.create(
                name="Test Med", dosage_mg=0, prescribed_per_day=1
            )
    ...
    def test_create_medication_negative_dosage(self):
        ...
        with self.assertRaises((IntegrityError, DataError, ValueError)):
            Medication.objects.create(
                name="Invalid Med", dosage_mg=-100, prescribed_per_day=1
            )
    ...
    def test_create_medication_zero_frequency(self):
        ...
        try:
            med = Medication.objects.create(
                name="Test Med", dosage_mg=100, prescribed_per_day=0
            )
    ...
    def test_create_medication_negative_frequency(self):
        ...
        with self.assertRaises((IntegrityError, DataError, ValueError)):
            Medication.objects.create(
                name="Invalid Med", dosage_mg=100, prescribed_per_day=-2
            )
    ...
    def test_create_medication_empty_name(self):
        ...
        try:
            med = Medication.objects.create(
                name="", dosage_mg=100, prescribed_per_day=2
            )
    ...
    def test_create_medication_very_high_dosage(self):
        ...
        med = Medication.objects.create(
            name="Special Treatment", dosage_mg=10000, prescribed_per_day=1
        )
    ...
    def test_adherence_rate_partial_completion(self):
        ...
        med = Medication.objects.create(
            name="Aspirin", dosage_mg=100, prescribed_per_day=4
        )
        ...
        DoseLog.objects.create(
            medication=med, taken_at=now - timedelta(hours=20), was_taken=False
        )
        DoseLog.objects.create(
            medication=med, taken_at=now - timedelta(hours=10), was_taken=True
        )
        DoseLog.objects.create(
            medication=med, taken_at=now - timedelta(hours=1), was_taken=False
        )
    ...
    def test_adherence_rate_all_missed(self):
        ...
        now = timezone.now()
        DoseLog.objects.create(
            medication=med, taken_at=now - timedelta(hours=30), was_taken=False
        )
        DoseLog.objects.create(
            medication=med, taken_at=now - timedelta(hours=20), was_taken=False
        )
        DoseLog.objects.create(
            medication=med, taken_at=now - timedelta(hours=10), was_taken=False
        )
    ...
    def test_adherence_rate_mixed_taken_missed(self):
        ...
        med = Medication.objects.create(
            name="Aspirin", dosage_mg=100, prescribed_per_day=5
        )
        ...
        for i in range(3):
            DoseLog.objects.create(
                medication=med,
                taken_at=now - timedelta(hours=(i + 1) * 10),
                was_taken=True,
            )
        for i in range(2):
            DoseLog.objects.create(
                medication=med,
                taken_at=now - timedelta(hours=(i + 4) * 10),
                was_taken=False,
            )
    ...
    def test_expected_doses_with_zero_prescribed(self):
        ...
        try:
            med = Medication.objects.create(
                name="As Needed", dosage_mg=100, prescribed_per_day=0
            )
...
class DoseLogModelTests(TestCase):
    ...
    def test_create_doselog_valid_data(self):
        ...
        log = DoseLog.objects.create(medication=self.med, taken_at=timezone.now())
    ...
    def test_create_doselog_explicit_was_taken(self):
        ...
        log_missed = DoseLog.objects.create(
            medication=self.med, taken_at=timezone.now(), was_taken=False
        )
    ...
    def test_create_doselog_past_date(self):
        ...
        log = DoseLog.objects.create(medication=self.med, taken_at=past_date)
    ...
    def test_create_doselog_future_date(self):
        ...
        log = DoseLog.objects.create(medication=self.med, taken_at=future_date)
    ...
    def test_create_doselog_no_medication(self):
        ...
        with self.assertRaises(Exception):
            DoseLog.objects.create(medication=None, taken_at=timezone.now())

    def test_create_doselog_no_timestamp(self):
        """Boundary testing: null timestamp (invalid)"""
        with self.assertRaises(Exception):
            DoseLog.objects.create(medication=self.med, taken_at=None)
    ...
    def test_doselog_str_representation_taken(self):
        ...
        log = DoseLog.objects.create(
            medication=self.med, taken_at=test_time, was_taken=True
        )
    ...
    def test_doselog_str_representation_missed(self):
        ...
        test_time = timezone.now()
        log = DoseLog.objects.create(
            medication=self.med, taken_at=test_time, was_taken=False
        )
...
class MedicationEquivalencePartitioningTests(TestCase):
    ...
    def test_frequency_equivalence_partitions(self):
        ...
        med1 = Medication.objects.create(
            name="Once Daily", dosage_mg=100, prescribed_per_day=1
        )
        self.assertEqual(med1.prescribed_per_day, 1)

        # Partition 2: Twice daily
        med2 = Medication.objects.create(
            name="Twice Daily", dosage_mg=100, prescribed_per_day=2
        )
        self.assertEqual(med2.prescribed_per_day, 2)

        # Partition 3: Three times daily
        med3 = Medication.objects.create(
            name="Three Times", dosage_mg=100, prescribed_per_day=3
        )
        self.assertEqual(med3.prescribed_per_day, 3)

        # Partition 4: Multiple times (4+)
        med4 = Medication.objects.create(
            name="Multiple Times", dosage_mg=100, prescribed_per_day=6
        )
...
class BoundaryValueTests(TestCase):
    ...
    def test_dosage_boundary_values(self):
        ...
        med1 = Medication.objects.create(
            name="Min Boundary", dosage_mg=1, prescribed_per_day=1
        )
        self.assertEqual(med1.dosage_mg, 1)

        # Normal value
        med2 = Medication.objects.create(
            name="Normal", dosage_mg=500, prescribed_per_day=1
        )
        self.assertEqual(med2.dosage_mg, 500)

        # High boundary
        med3 = Medication.objects.create(
            name="High Boundary", dosage_mg=9999, prescribed_per_day=1
        )
    ...
    def test_frequency_boundary_values(self):
        ...
        med1 = Medication.objects.create(
            name="Min Freq", dosage_mg=100, prescribed_per_day=1
        )
        self.assertEqual(med1.prescribed_per_day, 1)

        # Normal frequency
        med2 = Medication.objects.create(
            name="Normal Freq", dosage_mg=100, prescribed_per_day=3
        )
        self.assertEqual(med2.prescribed_per_day, 3)

        # High frequency boundary
        med3 = Medication.objects.create(
            name="High Freq", dosage_mg=100, prescribed_per_day=12
        )
    ...
    def test_expected_doses_boundary_values(self):
        ...
        med = Medication.objects.create(
            name="Test Med", dosage_mg=100, prescribed_per_day=2
        )
...
class MedicationModelEdgeCaseTests(TestCase):
    ...
    def test_fetch_external_info_exception_handling(self, mock_service):
        ...
        med = Medication.objects.create(
            name="Test Med", dosage_mg=100, prescribed_per_day=1
        )
...
class MedicationModelFinalCoverageTests(TestCase):
    ...
    def test_adherence_rate_over_period_exception_handling(self):
        ...
        med.prescribed_per_day = (
            0  # This would cause expected_doses to raise ValueError
        )
